...
class Player(metaclass=abc.ABCMeta):
    ...
    def play_turn(self, game_state: GameState) -> GameState:
        ...
        while will_roll_again and not farkled:
            roll_decision = self._play_dicehand(dice_hand, game_state)
            dice_hand = roll_decision.after_dicehand
            farkled = dice_hand.farkled
    ...
    @abc.abstractmethod
    def _play_dicehand(self, dice_hand, game_state: GameState) -> RollDecision:
...
class RobotPlayer(Player, metaclass=abc.ABCMeta):
    ...
    def _play_dicehand(self, dice_hand: DiceHand, game_state: GameState) -> RollDecision:
        time.sleep(self.delay_seconds)
        return self._robot_play_dicehand(dice_hand, game_state)

    @abc.abstractmethod
    def _robot_play_dicehand(self, dice_hand: DiceHand, game_state: GameState) -> RollDecision:
...
class RandomRobotPlayer(RobotPlayer):
    ...
    def _robot_play_dicehand(self, dice_hand: DiceHand, game_state: GameState) -> RollDecision:
        ...
        after_dicehand = dice_hand.copy()
        after_dicehand.lock_from_dicehand(score_decision)
        return RollDecision(dice_hand, after_dicehand, will_roll_again)
